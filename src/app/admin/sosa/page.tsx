...
export default function AdminSosaPage() {
  ...
  // 2. Determine if user can manage SOSA reports (President only)
  const canManage = officer ? RBAC.canManageSosa(officer.role as Role) : false;
  ...
  async function fetchReports() {
    ...
    try {
      ...
      toast.error("Failed to load SOSA reports.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function createReport(e: React.FormEvent) {
    ...
    const newReport: SosaReport = {
      ...
      departmental_progress: form.departmental_progress,
      house_performance: form.house_performance,
      presidential_vision: form.presidential_vision,
    ...
    setReports((prev) => [newReport as any, ...prev]);
    setShowForm(false);
    setForm(emptyForm);
    ...
    try {
      ...
      toast.success("SOSA report created.");
    } catch (err) {
      // Revert on failure
      setReports(previousReports);
      setShowForm(true); // Reopen form so user doesn't lose data
      toast.error("Failed to create SOSA report.");
    }
  }
  ...
  async function updateReport(id: number, updates: Partial<SosaReport>) {
    ...
    try {
      ...
      toast.success("SOSA report updated.");
    } catch (err) {
      // Revert on failure
      setReports(previousReports);
      toast.error("Failed to update SOSA report.");
    }
  }

  async function deleteReport(id: number) {
    if (!confirm("Are you sure you want to permanently delete this SOSA report?")) return;
    ...
    try {
      ...
      toast.success("SOSA report deleted.");
    } catch (err) {
      // Revert on failure
      setReports(previousReports);
      toast.error("Failed to delete SOSA report.");
    }
  }
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <label className="block text-sm font-medium text-neutral-300">President Name</label>
          <input
            type="text"
        ...
        <div className="space-y-2">
          <label className="block text-sm font-medium text-neutral-300">Academic Year</label>
          <input
            type="text"
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <label className="block text-sm font-medium text-neutral-300">Semester</label>
          <select
            value={form.semester}
        ...
        <div className="space-y-2">
          <label className="block text-sm font-medium text-neutral-300">Delivered Date</label>
          <input
            type="date"
      ...
      <div className="space-y-2">
        <label className="block text-sm font-medium text-neutral-300">Financial Health</label>
        <textarea
      ...
      <div className="space-y-2">
        <label className="block text-sm font-medium text-neutral-300">Departmental Progress</label>
        <textarea
      ...
      <div className="space-y-2">
        <label className="block text-sm font-medium text-neutral-300">House Performance</label>
        <textarea
      ...
      <div className="space-y-2">
        <label className="block text-sm font-medium text-neutral-300">Presidential Vision</label>
        <textarea
      ...
      <div className="space-y-2">
        <label className="block text-sm font-medium text-neutral-300">Additional Remarks</label>
        <textarea
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="space-y-2">
        <h1 className="text-2xl font-semibold text-white">State of the Society Address</h1>
        <p className="text-sm text-neutral-400">
          Manage SOSA reports delivered by the President per Constitution Art. 8, Sec. 4(i). Published reports appear on the public SOSA archive.
        </p>
      </div>
      ...
      {!canManage && officer && (
        <div className="rounded-2xl border border-amber-900/40 bg-amber-950/20 p-5">
          <p className="text-sm leading-6 text-amber-300/80">
            <strong className="text-amber-200">View Only:</strong> Per Constitution Article 8, Section 3(i), 
            only the <strong className="text-white">Society President</strong> can draft, edit, or publish 
            the State of the Society Address. You may view the reports for review and archiving purposes.
          </p>
        </div>
      )}
      ...
      {/* New Report Button (RBAC Protected) */}
      {fetched && !showForm && editingId === null && canManage && (
        <button
          onClick={startNew}
          className="rounded-full bg-neutral-100 px-6 py-2.5 text-sm font-semibold text-neutral-950 transition hover:bg-neutral-200"
        >
      ...
      {/* Form (RBAC Protected) */}
      {(showForm || editingId !== null) && canManage && (
        ...
        <div className="space-y-4">
          {reports.map((report) => (
            ...
            >
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                ...
                {canManage && (
                  <div className="flex gap-2">
                    ...
                    {!report.is_published && (
                      <button
                        onClick={() => updateReport(report.id, { is_published: true })}
                    ...
                    {report.is_published && (
                      <button
                        onClick={() => updateReport(report.id, { is_published: false })}
                        className="rounded-full bg-neutral-800 px-4 py-1.5 text-xs font-semibold text-neutral-300 transition hover:bg-neutral-700"
                      >
                        Unpublish
                      </button>
              ...
              <div className="mt-4 grid grid-cols-1 gap-1 text-xs text-neutral-500 sm:grid-cols-2">
